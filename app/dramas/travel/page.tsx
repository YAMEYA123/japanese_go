...
export default function TravelListPage() {
  ...
  return (
    <div className="pb-24">
      <div className="bg-gradient-to-br from-teal-700 to-teal-600 px-5 pt-12 pb-6 relative">
        <Link href="/dramas" className="absolute top-4 left-4 text-white/80 text-sm">← 返回</Link>
        <div className="w-10 h-10 rounded-xl bg-white/15 flex items-center justify-center mb-3">
          <Map size={20} className="text-white" />
        </div>
        <h1 className="text-white text-2xl font-bold" style={{ fontFamily: 'Noto Serif JP, serif' }}>旅游场景</h1>
        <p className="text-white/60 text-sm mt-1">餐厅、交通、购物、住宿等实用场景 · {travel.length} 个</p>
      </div>

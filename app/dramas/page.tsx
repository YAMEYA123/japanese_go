...
export default function DramasPage() {
  const { selectedDramaIds, selectDrama, deselectDrama, srsCards } = useAppStore()
  ...
  return (
    <div className="px-4 pt-8 pb-4">
      <div className="mb-6">
        ...
        <p className="text-stone-500 text-sm mt-1">选择你在追的剧，优先学剧中词汇</p>
      </div>
      ...
      <div className="space-y-3">
        {DRAMAS.map(drama => {
          ...
          const selected = selectedDramaIds.includes(drama.id)

          return (
            <div
              key={drama.id}
              className={`bg-white rounded-2xl shadow-sm border overflow-hidden transition-all ${
                selected ? 'border-red-200' : 'border-stone-100'
              }`}
            >
              <div className="flex items-start p-4 gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <p className="font-bold text-stone-900" style={{ fontFamily: 'Noto Serif JP, serif' }}>
                        {drama.title_jp}
                      </p>
                      <p className="text-stone-500 text-xs">{drama.title_zh} · {drama.year}</p>
                    </div>
                    <button
                      onClick={() => selected ? deselectDrama(drama.id) : selectDrama(drama.id)}
              ...
              {words.length > 0 && (
                <Link
                  ...
                  className="flex items-center justify-between px-4 py-2.5 border-t border-stone-50 text-xs text-red-600 font-medium"
                >
